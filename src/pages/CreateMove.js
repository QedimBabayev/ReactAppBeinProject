...
class CreateMove extends Component {
    ...
    render() {
        ...
        const returnElementId = (
            this.state.doc != '' ?
                <div>
                    <DocButtons buttonsName={buttonsNames} from = {'p=move'} activeitem={this.props.state.navbar.activeItem} activesubitem={this.props.state.navbar.activeSubItem} />
                    <CreateMoveForm datas={this.props.state.groups.groups} selectedDoc={this.state.doc} />
                </div>
                : <div>Loading..</div>
        )
        const returnElement = (
